...
def run_all_backtests(mock_sp500, mock_weekly):
    ...
    if all_results:
        ...
        print("═" * 70)
        print(f"  {'Symbol':<8} {'BUYs':>6} {'Wins':>6} {'Win%':>8} {'AvgRet%':>9} {'System%':>9} {'B&H%':>9} {'vs B&H':>9}")
        print("─" * 70)
        ...
        for r in all_results:
            ...
            if buys > 0:
                ...
                vs_bh = r.get("system_vs_benchmark", 0)
                print(f"  {sym:<8} {buys:>6} {wins:>6} {wr:>7.1%} {avg:>+8.2f}% {sys_ret:>+8.2f}% {bh:>+8.2f}% {vs_bh:>+8.2f}%")
                total_buys += buys
                total_wins += wins
                all_returns.extend([t["actual_return_pct"] for t in r["trades"]])
            else:
                print(f"  {sym:<8} {buys:>6}      -        -         -         -  {bh:>+8.2f}%         -")

        print("─" * 70)
        if total_buys > 0:
            overall_wr = total_wins / total_buys
            avg_all = sum(all_returns) / len(all_returns)
            print(f"  {'TOTAL':<8} {total_buys:>6} {total_wins:>6} {overall_wr:>7.1%} {avg_all:>+8.2f}%")
        print("═" * 70)
